...
const CustomModal = ({
  ...
  return (
    <>
      <div
        className={`bg-white flex-col h-fit max-h-[90%] w-[800px] max-w-[90%] px-[27px] py-[23px] flex rounded-lg fixed ${
      ...
      >
        <div className="flex flex-col w-full flex-1 overflow-y-auto">
          <div className="flex flex-row w-full items-center h-fit mb-[23px]">
            <h1 className={`flex-1 text-[18px] font-semibold text-md`}>
              {title}
            </h1>
            <div onClick={setShow}>
              <svg
          ...
          <div className="flex flex-col overflow-y-auto gap-4 w-full overflow-x-hidden">
            {children}
          </div>
        </div>
        {actions && actions}
